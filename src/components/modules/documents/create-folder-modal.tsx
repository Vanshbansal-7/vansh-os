...
import { X, FolderPlus, Plus, AlertCircle } from "lucide-react";

interface CreateFolderModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSuccess: (name: string) => Promise<any> | any;
}
...
export function CreateFolderModal({ isOpen, onClose, onSuccess }: CreateFolderModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.error("[CreateFolderModal] Submit failed:", err);
      setErrorMsg(err?.message || "Failed to create folder in database.");
    } finally {
      ...
      <div className="w-full max-w-md rounded-2xl bg-[#0E101A] border border-purple-500/30 shadow-[0_0_40px_rgba(168,85,247,0.25)] overflow-hidden">
        <div className="flex items-center justify-between px-5 py-4 bg-[#131626] border-b border-white/[0.08]">
          <div className="flex items-center gap-2.5">
            ...
            <h3 className="text-sm font-bold text-white tracking-tight">Create New Folder</h3>
          </div>
        ...
        <form onSubmit={handleSubmit} className="p-5 flex flex-col gap-4">
          ...
          <div className="flex flex-col gap-1.5">
            ...
            <input
              ...
              placeholder="e.g. System Design Notes, Placement Receipts..."
